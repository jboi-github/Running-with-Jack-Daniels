...
private func posX(from dataRect: CGRect, _ value: Double, to canvasRect: CGRect) -> CGFloat {
    let p = (dataRect.minX ..< dataRect.maxX).p(value)
    return (canvasRect.minX ..< canvasRect.maxX).mid(p)
}

private func posY(from dataRect: CGRect, _ value: Double, to canvasRect: CGRect) -> CGFloat {
    let p = (dataRect.minY ..< dataRect.maxY).p(value)
    return canvasRect.height / 2 - (canvasRect.minY ..< canvasRect.maxY).mid(p)
}
