...
struct RunView: View {
    ...
    var body: some View {
        VStack {
            List {
                Section("Totals") {
                    TotalsView(totals: timeseriesSet.totals)
                }
                Section("\(Date.now.formatted())") {
                    VStack(spacing: 0) {
                        Group {
                            HStack {
                ...
                Section("Motion Acitivity") {
                    HStack {
                        if let last = AppTwin.shared.timeseriesSet.motionActivityTimeseries.elements.last {
                            ...
                            Text("\(last.date.formatted())")
                            Spacer()
                            MotionActivityView(motion: last.motion, confidence: last.confidence)
                        }
                        Spacer()
                        Text(Image(systemName: AppTwin.shared.sensorClients[2].status.systemName))
                        Text("\(AppTwin.shared.timeseriesSet.motionActivityTimeseries.elements.count)")
            ...
            .onReceive(timer) {
                AppTwin.shared.timeseriesSet.refreshTotals(upTo: $0)
            }
            Button {
                if workoutIsStarted {
...
private struct TotalsView: View {
    ...
    var body: some View {
        VStack {
            Text(totals.count.formatted)
            ForEach(totals) { total in
                Group { // Category
                    HStack {
                        Text("isWorkingOutSince")
                        Spacer()
                        Text("\(total.workoutDate?.formatted() ?? "--:--")")
                    }
                    HStack {
                        Text("isWorkingOut")
                        Spacer()
                        Text("\(total.isWorkingOut.formatted)")
                    }
                    HStack {
                        Text("isActive")
                        Spacer()
                        MotionActivityView(motion: total.motionActivity, confidence: nil)
                    }
                }
                .foregroundColor(total.intensity?.textColor)
                .padding()
                .background(total.intensity?.color)
                
                Group { // Measures
                    HStack {
                        Text("numberOfSteps")
                        Spacer()
                        Text("\(Int(total.numberOfSteps ?? 0 + 0.5).formatted)")
                    }
                    HStack {
                        Text("pdmDistance")
                        Spacer()
                        DistanceText(distance: total.pdmDistance)
                    }
                    HStack {
                        Text("gpsDistance")
                        Spacer()
                        DistanceText(distance: total.gpsDistance)
                    }
                    HStack {
                        Text("Duration")
                        Spacer()
                    ...
                    HStack {
                        Text("activeDuration")
                        Spacer()
                        TimeText(time: total.activeDuration)
                    }
                    HStack {
                        Text("heartrateSeconds")
                        Spacer()
                        Text("\(total.heartrateSeconds ?? .nan, specifier: "%4.1f")")
                    }
                }
            }
        }
