...
enum Run {
    static func bmi(weightKg: Double, heightM: Double) -> Double {weightKg / (heightM * heightM)}
    ...
    static func hrMaxBpm(birthday: Date, gender: Gender) -> Int? {
    ...
    /// - Returns: A rough estimation of max heartrate. `nil`, if gender is `.none`
    static func hrMaxBpm(birthday: Date, gender: Gender, weightKg: Double) -> Int? {
        guard gender != .other else {return nil}
    ...
    static func hrLimits(hrMaxBpm: Int, restingHrBpm: Int = 0) -> [Intensity: Range<Int>] {
    ...
    enum Intensity: String, CaseIterable, Identifiable, Codable, Comparable {
        ...
        func getHrLimit(hrMaxBpm: Int, restingBpm: Int = 0) -> Range<Int>? {
        ...
        func getHrBuckets(for limits: [Intensity: Range<Int>]) -> [Intensity: Range<Int>] {
    ...
    static func pace4VdotPercent(vdot: Double, percent: Double) -> TimeInterval {
    ...
    /// - Returns: pace in seconds per km
    static func vdot4PacePercent(paceSecPerKm: TimeInterval, percent: Double) -> Double {
    ...
    static func vdot4DistTime(distanceM: Double, timeSec: TimeInterval) -> Double {
        let timeMinD = Double(timeSec) / 60.0
        let v = Double(distanceM) / timeMinD
        
        let percentMax = 0.8 + 0.1894393 * exp(-0.012778 * timeMinD) + 0.2989558 * exp(-0.1932605 * timeMinD)
        let vo2 = -4.60 + 0.182258 * v + 0.000104 * v * v
    ...
    static func dist4VdotTime(vdot: Double, timeSec: TimeInterval) -> Double {
        let timeMinD = Double(timeSec) / 60.0
    ...
    static func time4VdotDist(vdot: Double, distanceM: Double) -> TimeInterval {
    ...
    static func planRace(vdot: Double, distanceM: Double) -> TimeInterval {
    ...
    static func planTraining(vdot: Double) -> [Intensity : (lower: TimeInterval, upper: TimeInterval)] {
    ...
    static func train(hrBpm: Int, paceSecPerKm: TimeInterval, limits: [Intensity: Range<Int>]) -> Double? {
    ...
    static func vdot4Race(distanceM: Double, timeSec: TimeInterval) -> Double {
